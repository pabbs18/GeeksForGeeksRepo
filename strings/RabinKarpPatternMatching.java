...
public class RabinKarpPatternMatching {
	...
	public static void rabinKarp(String text, String pattern) {
		...
		for (int i = 0; i < m; i++) {
			patternHash += pattern.charAt(i) * Math.pow(base, m - i - 1);
		}
		...
		long textHash = 0;
		for (int i = 0; i < m; i++) {
			textHash += text.charAt(i) * Math.pow(base, m - i - 1);
		}
		...
		for (i = 0; i < n - m; i++) {
			...
			if (i < n - m) {// as long as i is within the bounds
				...
				textHash = (textHash - (text.charAt(i) * Math.round(Math.pow(base, m - 1)))// remove leading character
				) * base // multiply it with 10(or other base)
						+ (text.charAt(i + m));//add next character
